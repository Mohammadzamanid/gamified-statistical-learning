...
describe("skeleton honesty", () => {
  ...
  it("leaves every lesson not declared Complete looking like a skeleton", () => {
    ...
    //
    // Between S2-12 and S2-14 a skeleton could also hold inherited Stage 1
    // content staged for it, declared in tests/helpers/staged-inherited.ts and
    // bounded by a block of checks here. S2-14 cycle 4 gave the last staged
    // question a role, which emptied the map — and the mechanism's own audit
    ...
    for (const lesson of seeded) {
      expect(
        ...
        `${lesson.id} has grown beyond its seed question — declare it Complete`
      ).toBe(1);
