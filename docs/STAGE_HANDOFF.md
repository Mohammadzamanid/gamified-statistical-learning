...
`-`, not `−`. If a demonstration needs an operation the formula enum lacks, grow the enum: the readout switch is
exhaustive, so the compiler finds the second half (D-042).
