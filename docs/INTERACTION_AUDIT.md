...
**F-6 — One authored question declares a misconception the engine can never report. → CLOSED by S2-12.**
`q.error-id-causation` declared `mc.correlation-causation`, whose detector is `tagged-distractor`, but none of its three
options carried that tag. The declaration therefore inflated a mapping count while a learner who got it wrong received a
bare "incorrect" rather than the remediation.
...
authored misconception declarations. 45 could be reported; this one could not.

The finding was filed as needing the question's options redesigned, on the reading that "in that question the
misconception *is* the correct answer, so there is no distractor for it to sit on". **That reading was wrong**, and
saying so is the useful half of the correction. The question asks what the *flaw* in a causal claim is. A learner who
believes correlation establishes causation does not see the causal leap as a flaw at all, so they go hunting for a
different one — and `ch.sample`, "too few harbors were surveyed", is exactly the answer that belief produces. Tagging it
needed no redesign at all. S2-12 tagged it while re-cutting the lessons this question moved between, and all 46
authored declarations are now engine-reportable.
